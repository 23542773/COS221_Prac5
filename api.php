...
function handleLogout($data) {
    ...
    $conn = null;
    try {
        // Create a database connection inside this function to avoid global variable issues
        $conn = new mysqli(
            DBConfig::$host,
            DBConfig::$user,
            DBConfig::$pass,
        ...
        // API key is valid, log out the user
        // In a more sophisticated system, you might:
        // 1. Invalidate this API key
        ...
        $response['timestamp'] = time();
        //close the connection of prepared varaible that have the prepared statement and atabase connection
        $stmt->close();
        $conn->close();
        //convert the response php array to JSON and send it
        echo json_encode($response);
...
function handleGetAllRetailers($data) {
    ...
    $response['status'] = 'success';
    $response['dat  a'] = []; // Populate with retailer data
