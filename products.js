...
function populateProductDetails(data) {
    ...
    cartBtn.onclick = () => {
        const productId = data?.prices?.[0]?.ProductID;
        if (!productId) {
            console.error("Missing product ID for cart:", data?.prices);
            alert("Could not add to cart: Missing product ID.");
            return;
        }
        addToCart(productId);
    };//add to cart
...
async function addToCart(productId) {
    const data = {
        api: "addToCart",
        apikey: localStorage.getItem('apikey') || "dafdda51e1bf23147967c1041cac5d6b",
        operation: "set",
        PID: productId,
        Quantity: 1
    };
